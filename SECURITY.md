...
- **权限控制**：API Key 以明文保存于 `credentials.json`。Unix 文件权限为 `0600`；Windows 开发版使用受保护的 DACL，仅允许当前用户、Administrators 和 SYSTEM 访问。这是本机访问控制，不是静态加密；若检测到权限过宽，启动时将自动收紧权限并给出警告。
