...
[ 用户终端 ]                                   [ TokenFlux Web 网页前端 ]
     │                                                     │
1. 运行 `tf login [名字] --from-web [--host <网关>]`         │
...
- `--from-web`（必需）：开启本地回环 HTTP 导入监听。不能与 `--with-key` 同时使用。
...
- **生命周期**：仅在 `tf login --from-web` 执行期间短暂开启。用户拒绝请求后继续监听；用户确认一个请求后关闭 HTTP 服务并继续网关校验和写盘。服务**不常驻后台**。
...
2. **终端交互确认绝不可绕过**
   ...
   - CLI 明确设计：`--json` 或 `--no-input` 标志**不会且不能**绕过该终端确认。在非交互终端下使用 `--from-web` 会直接报错退出。
...
1. **不支持远程/SSH 环境的网页直连**
   - 协议基于浏览器到本机回环地址（`127.0.0.1`）的通信。若用户通过 SSH 在远程服务器运行 `tf login --from-web`，本地浏览器访问本地 `127.0.0.1` 将无法触达远端 CLI（除非用户自行配置了 SSH 端口转发）。此类场景请引导用户使用常规的 `tf login` / `tf login --with-key` 隐藏输入粘贴。
