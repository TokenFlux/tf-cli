...
2. 对 `claude`、`codex`、`opencode` 三个透传命令，遇到首个非内置参数时，后续参数全部原样交给底层子进程。
3. `--` 之后的内容无条件全量透传。
...
- 模型列表、协议探测结果与 `probed_at` 保存在 `config.json` 的 Key 元数据中，正常启动直接复用。
- 不设置 TTL。启动失败时触发重探；`tf keys --refresh` 可显式刷新。网络失败时保留已有结果。
