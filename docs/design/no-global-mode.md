...
结果连同模型列表一起存进 `config.json` 的 `KeyMeta`。正常完整启动只读本地；启动筛选从缓存找不到可用 Key 时，先刷新模型目录，再按最新前缀重探协议。

## 边界

- 探测结果**只能证伪**：账号级 endpoint capability 可能更窄，通过不代表一定能跑。
  因此候选为空时才拦，候选非空时静默放行，绝不给「配置正确」的承诺。
- 能力会变（用户改分组）。早期曾计划使用 TTL，现行实现不设 TTL；启动筛选从缓存找不到可用 Key 时，先刷新模型目录，再按最新前缀重探协议。
...
「这把 Key 能跑什么」，于是一把 GPT+Claude 的复合 Key 会显示能跑所有兼容
