...
> 状态：待实现。早期文稿里的“v0.5”指过渡阶段，不对应已发布的 SemVer `v0.5.0`。
...
安全兵力全部压在**终端侧带 Origin 的预览确认**一处：不管请求来自哪个 Origin、连的哪个端口，写盘前都要用户确认。它同时盖住了「导错终端」和「恶意页面推 Key」，因此不再需要额外的配对机制。
...
2. **过渡版**：keys 页加「导入 tf」按钮 + localhost 通道。改 CLI 与前端，不改 TokenRouter 后端，把主流程做到一键。
